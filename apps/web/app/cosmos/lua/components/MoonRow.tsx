...
const MoonRow: React.FC<MoonRowProps> = ({
  ...
  const renderMoonItems = (moonItems: MoonRowItem[]) => {
    return moonItems.map(({ month, phase }, idx) => {
      ...
      const floatDirection = isNewPhase ? 'up' : 'down';
  ...
  return (
    <div className="flex w-full flex-col items-center gap-8 py-3 xs:gap-10 sm:gap-14 sm:py-4 md:gap-16 lg:gap-20 lg:py-5">
      {/* Linha de Luas Novas */}
      <div
      ...
      >
        {renderMoonItems(newMoons)}
      </div>

      {/* Linha de Luas Cheias */}
      <div
        className="flex min-w-max items-center"
        style={{
          columnGap: gap,
