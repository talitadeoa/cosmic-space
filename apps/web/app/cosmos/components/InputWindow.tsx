...
const baseClasses = 'relative border';

const variantClasses: Record<InputWindowVariant, string> = {
  glass: 'border-white/15 bg-white/10 shadow-2xl backdrop-blur-lg',
  nebula:
    'border-slate-800 bg-gradient-to-br from-slate-950/70 via-slate-900/60 to-indigo-950/40 shadow-xl shadow-indigo-900/20',
};
...
const InputWindow: React.FC<InputWindowProps> = ({
  ...
  return (
    ...
    >
      {showAccent && <div className={`pointer-events-none absolute inset-0 ${accentClassName}`} />}
      {children}
    </div>
  );
};
