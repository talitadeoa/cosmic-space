...
/**
 * Adiciona uma mudança à fila de sincronização (outbox).
 */
...
/**
 * Lista mudanças pendentes na fila por tipo.
 */
...
export const removeOutboxChange = async (clientChangeId: string) => {
  await withStore(OUTBOX_STORE, 'readwrite', (store) => {
    store.delete(clientChangeId);
  });
};

/**
 * Atualiza o status de uma mudança na fila.
 */
...
/**
 * Obtém metadados do armazenamento.
 */
...
/**
 * Define metadados no armazenamento.
 */
...
/**
 * Limpa todos os dados de sincronização (outbox e meta).
 */
