import { MOON_PHASE_EMOJI_LABELS, type MoonPhase } from './moonPhases';
...
export const QUARTERLY_INFO: Record<MoonPhase, { name: string; quarter: string; months: string }> =
  {
...
/**
 * Construir chave de armazenamento local genérica
 */
export const buildStorageKey = (
  type: 'mensal' | 'trimestral' | 'anual' | 'energia-ring',
  ...values: (string | number)[]
): string => {
  return [`insight-${type}`, ...values].join('-');
};
...
export const buildMonthlyStorageKey = (
  ...
  monthNumber: number,
  phase: MoonPhase
) => buildStorageKey('mensal', year, monthNumber, phase);

export const buildQuarterlyStorageKey = (year: number, phase: MoonPhase) =>
  buildStorageKey('trimestral', year, phase);

export const buildAnnualStorageKey = (year: number) => buildStorageKey('anual', year);

export const buildRingEnergyStorageKey = (phase: MoonPhase) =>
  buildStorageKey('energia-ring', phase);
