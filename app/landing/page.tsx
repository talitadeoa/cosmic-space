      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-20">
        <section className="grid gap-10 lg:grid-cols-[1.5fr_1fr]">
          <div className="rounded-[32px] border border-white/10 bg-black/30 p-8 shadow-2xl shadow-indigo-900/30 backdrop-blur-xl">
            ...
            <p className="mt-4 text-base text-slate-300 sm:text-lg">
              Uma landing viva para acompanhar o nascimento do hub que sincroniza Sol, Lua, Planeta e
              toda a galáxia produtiva. Entre na lista para receber os bastidores e o kit de
              integração.
            </p>
          ...
          <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-gradient-to-br from-space-medium/70 via-slate-900/50 to-space-dark/90 p-8 shadow-xl">
            ...
            <div className="relative space-y-6">
              ...
              <p className="text-3xl font-semibold text-white">Narrativa viva, camadas sonoras e visual tático</p>
              <div className="space-y-5">
                {signalBeams.slice(0, 2).map((beam) => (
                  <div key={beam.label} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-5">
                    <p className="text-xs uppercase tracking-[0.3em] text-slate-300">{beam.label}</p>
              ...
              <div className="rounded-2xl border border-indigo-400/30 bg-indigo-500/10 px-4 py-5">
                <p className="text-xs uppercase tracking-[0.3em] text-indigo-200">Envelope sonoro</p>
                <p className="mt-2 text-lg text-white">Radio Player + Sfx Provider ativados para guiar cada salto.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-400">ferramentas orbitais</p>
              <h2 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">Recursos feitos para explorar cada camada</h2>
            </div>
            <p className="text-sm text-slate-400 sm:max-w-sm">
              Tudo conversa entre si: insights, áudio, alerts, automações e os fluxos visuais que você já
              conhece nas rotas internas.
            </p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2">
            {highlightCards.map((card) => (
              ...
              >
                <div className={`absolute inset-0 opacity-0 blur-3xl transition group-hover:opacity-40 bg-gradient-to-r ${card.accent}`} />
        ...
        <section className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[32px] border border-white/10 bg-black/30 p-8 shadow-2xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-slate-400">linha do lançamento</p>
              ...
              <ul className="space-y-8">
                {launchTimeline.map((step, index) => (
                  <li key={step.phase} className="relative pl-12">
                    ...
                    <p className="text-xs uppercase tracking-[0.3em] text-slate-400">fase {step.phase}</p>
                    <div className="mt-2 flex items-center gap-2 text-sm text-slate-400">
                      <span>{step.emoji}</span>
                      <span>{step.status}</span>
                      {index === 1 && <span className="animate-pulse text-emerald-300">• ao vivo</span>}
                    </div>
          ...
          <div className="rounded-[32px] border border-white/10 bg-gradient-to-b from-slate-900/80 via-space-medium/60 to-space-dark/80 p-8 shadow-xl">
            <p className="text-xs uppercase tracking-[0.35em] text-slate-400">frequência de sinais</p>
            ...
            <div className="mt-8 space-y-6">
              {signalBeams.map((beam) => (
                <div key={beam.label} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <p className="text-[11px] uppercase tracking-[0.3em] text-slate-300">{beam.label}</p>
        ...
        <section className="relative overflow-hidden rounded-[36px] border border-white/15 bg-gradient-to-br from-indigo-900/60 via-space-medium/80 to-black/90 p-10 shadow-2xl">
          ...
          <div className="relative grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-5">
              <p className="text-xs uppercase tracking-[0.4em] text-slate-300">lista de transmissão</p>
              <h2 className="text-4xl font-semibold text-white">Receba o kit de orbitas e a playlist de lançamento</h2>
              ...
              <div className="flex flex-wrap gap-3 text-sm text-slate-300">
                <span className="rounded-full border border-white/20 px-3 py-1">🎧 Radio Player</span>
                <span className="rounded-full border border-white/20 px-3 py-1">🛰️ Neon branch</span>
                <span className="rounded-full border border-white/20 px-3 py-1">🌗 Guia Luações</span>
              </div>
            </div>
