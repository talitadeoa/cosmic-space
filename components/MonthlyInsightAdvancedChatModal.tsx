...
interface MonthlyInsightAdvancedChatModalProps {
  isOpen: boolean;
  moonIndex: number;
...
const moonPhaseColors: Record<string, { bg: string; border: string; text: string }> = {
  luaNova: { bg: 'bg-indigo-950', border: 'border-indigo-700', text: 'text-indigo-100' },
  luaCrescente: { bg: 'bg-blue-950', border: 'border-blue-700', text: 'text-blue-100' },
  luaCheia: { bg: 'bg-amber-950', border: 'border-amber-700', text: 'text-amber-100' },
  luaMinguante: { bg: 'bg-slate-900', border: 'border-slate-700', text: 'text-slate-100' },
};
...
export default function MonthlyInsightAdvancedChatModal({
  isOpen,
  moonIndex,
  moonPhase,
  moonSignLabel,
  ...
  onClose,
  onSubmit,
}: MonthlyInsightAdvancedChatModalProps) {
  ...
  const phaseColor = moonPhaseColors[moonPhase];
        ...
        >
          ...
          >
            <InputWindow variant="glass" size="md" radius="lg" showAccent className="flex flex-col h-[600px]">
              ...
              {/* Cabeçalho */}
              <div className={`border-b ${phaseColor.border} pb-4`}>
                <div className={`mb-2 text-sm font-semibold uppercase tracking-[0.24em] ${phaseColor.text}/80`}>
                  {phaseLabel}
                </div>
                <h2 className="text-2xl font-bold text-white">{monthName}</h2>
                <p className="text-xs text-slate-200/70">Mês #{monthNum}</p>
                <div className={`mt-2 inline-flex items-center justify-center gap-2 rounded-full border ${phaseColor.border} ${phaseColor.bg} px-3 py-1 text-[11px] font-medium uppercase tracking-[0.12em] ${phaseColor.text}`}>
                  <span>Signo</span>
              ...
              <div className="flex-1 overflow-y-auto space-y-4 py-4 px-2 scrollbar-thin scrollbar-track-white/5 scrollbar-thumb-white/20">
                {messages.map((message, index) => (
                  ...
                  >
                    <div
                      className={`max-w-xs rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap ${
                        message.role === 'user'
              ...
              {hasUserMessage && (
                ...
                >
                  {messageCount} mensagem{messageCount !== 1 ? 's' : ''} registrada{messageCount !== 1 ? 's' : ''}
                </motion.div>
              )}
