...
export default function CycleTracker({
  ...
  const [records, setRecords] = useState<CycleRecord[]>(existingRecords);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('cycle_records');
    if (saved) {
      try {
  ...
  const { data: moonData, isLoading: moonLoading } = useLunarPhase(selectedDate, { includeZodiac: true });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    localStorage.setItem('cycle_records', JSON.stringify(allRecords));
            ...
            {records.map((record) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold text-pink-200">{record.cycleDate}</p>
